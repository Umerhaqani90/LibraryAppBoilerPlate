...
using BoilerPlateTest.Migrations.SeedData;
using BoilerPlateTest.MultiTenancy;
using BoilerPlateTest.Users;
...
{
    public abstract class BoilerPlateTestTestBase : AbpIntegratedTestBase<BoilerPlateTestTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected BoilerPlateTestTestBase()
        {
        ...
        protected void UsingDbContext(Action<BoilerPlateTestDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BoilerPlateTestDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BoilerPlateTestDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BoilerPlateTestDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BoilerPlateTestDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerPlateTestDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BoilerPlateTestDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BoilerPlateTestDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BoilerPlateTestDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BoilerPlateTestDbContext, Task<T>> func)
        {
            T result;
